...
virtual void play () {
    ...
    while(not lenin.empty()) {
        ...
        vector<vector<pair<int, pair<int, int> > > > dist2(rows(), vector<pair<int, pair<int, int> > > (cols(), make_pair(-1, make_pair(-1, -1))));
        priority_queue<pair<pair<int, int>, pair<pair<int, int>, pair<int, int> > > > q2;
        q2.push(make_pair(make_pair(0, 0), make_pair(make_pair(posx, posy), make_pair(posx, posy))));
        while(!q2.empty()) {
            int dist=-q2.top().first.first;
            int hp=-q2.top().first.second;
            pair<int, int> to=q2.top().second.first;
            pair<int, int> from=q2.top().second.second;
            q2.pop();
            if (dist2[to.first][to.second].first==-1) {
                ...
                for (int k=0; k<4; ++k) {
                    if (puc(to.first+dirx[k], to.second+diry[k]) and dist<3 and dist2[to.first+dirx[k]][to.second+diry[k]].first==-1) {
                        q2.push(make_pair(make_pair(-(dist+1), -(hp+cost(cell(to.first+dirx[k], to.second+diry[k]).type))), make_pair(make_pair(to.first+dirx[k], to.second+diry[k]), to)));
                    }
                }
            }
        }
